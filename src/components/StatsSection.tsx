...
import type { CompanyStat, CompanyStatIconKey } from "@/data/stats";
...
/* The figures live in `data/stats` so the homepage and About page can never
   drift apart; only the icon mapping stays here, on the client side. */
const ICONS: Record<CompanyStatIconKey, LucideIcon> = {
  turnover: TrendingUp,
  employees: Users,
  patents: Award,
  countries: Globe2,
...
/* Counts 0 → count once the number scrolls into view. Writes straight to the
   DOM node so the 60fps tween never triggers a React re-render. */
function Counter({
  count,
  prefix = "",
  suffix = "",
}: Pick<CompanyStat, "count" | "prefix" | "suffix">) {
      ...
      <div className="shell">
        <div className="relative">
          ...
          <div className="rounded-[22px] bg-gradient-to-b from-line via-line/50 to-line/20 p-px shadow-[0_30px_80px_-24px_rgba(0,0,0,0.95)]">
            ...
            >
              {COMPANY_STATS.map(({ icon, prefix, count, suffix, label }) => {
                    ...
                    <div className="relative flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <p className="font-display text-3xl font-bold leading-none tracking-[-0.03em] text-white sm:text-[2rem] lg:text-4xl">
                          <Counter count={count} prefix={prefix} suffix={suffix} />
                        </p>
